...
with tab3:
    ...
    st.markdown(f"""
        <div class="yc-hero">
            <div class="yc-hero-kicker">Executive Talent Analytics</div>
            <div class="yc-hero-title">Year-over-Year Talent Health</div>
            <div class="yc-hero-sub">A simplified executive view of how talent quality, risk exposure, performance, and grid movement changed across 2023, 2024, and 2025.</div>
        </div>
    """, unsafe_allow_html=True)
    ...
    for col, (lbl, val, sub, dlt) in zip([c1, c2, c3, c4], cards):
        with col:
            st.markdown(f"""
                <div class="yc-card">
                    <div class="yc-label">{lbl}</div>
                    <div class="yc-value">{val}</div>
    ...
    with left_col:
        ...
        if top_transitions.empty:
            ...
            for _, r in top_transitions.iterrows():
                move_rows_html += f"""
                <div class="move-row">
                    <div><div class="move-caption">From</div><div class="move-from">{r['from_label']}</div></div>
                    <div class="move-arrow">→</div>
                    <div><div class="move-caption">To</div><div class="move-to">{r['to_label']}</div></div>
                    <div class="move-count">{int(r['count'])}<small>people</small></div>
                </div>
                """
        st.markdown(f"""
            <div class="yc-panel">
                <div class="yc-panel-title"><h3>Talent Movement</h3><span>2024 → 2025</span></div>
                {move_rows_html}
            </div>
        """, unsafe_allow_html=True)
    ...
    with right_col:
        ...
        improved_pct = improved / total_move * 100
        declined_pct = declined / total_move * 100
        stable_pct = stable / total_move * 100
        movement_summary_html = f"""
        <div class="trend-row"><div class="trend-name">Improved</div><div><div class="bar-line"><div class="bar-year">%</div><div class="bar-track"><div class="bar-fill" style="width:{improved_pct:.1f}%"></div></div><div class="bar-val">{improved_pct:.0f}%</div></div></div></div>
        <div class="trend-row"><div class="trend-name">Stable</div><div><div class="bar-line"><div class="bar-year">%</div><div class="bar-track"><div class="bar-fill" style="width:{stable_pct:.1f}%;background:linear-gradient(90deg,#98A2B3,#667085)"></div></div><div class="bar-val">{stable_pct:.0f}%</div></div></div></div>
        <div class="trend-row"><div class="trend-name">Declined</div><div><div class="bar-line"><div class="bar-year">%</div><div class="bar-track"><div class="bar-fill" style="width:{declined_pct:.1f}%;background:linear-gradient(90deg,#F97316,#EF4444)"></div></div><div class="bar-val">{declined_pct:.0f}%</div></div></div></div>
        """
        st.markdown(f"""
            <div class="yc-panel">
                <div class="yc-panel-title"><h3>Movement Health</h3><span>{len(move_df)} matched employees</span></div>
    ...
    with col_trend:
        st.markdown(f"""
            <div class="yc-panel">
                <div class="yc-panel-title"><h3>Talent Distribution Trend</h3><span>Headcount by year</span></div>
                {trend_html}
            </div>
        """, unsafe_allow_html=True)
        ...
        if latest_df.empty:
            ...
            for medal, (_, emp) in zip(["1", "2", "3"], latest_df.iterrows()):
                ...
                <div class="spotlight-card">
                    ...
                    <div><div class="spot-name">{emp.get('name','—')}</div><div class="spot-meta">{emp.get('position','—') or '—'}<br>{emp.get('dept','—') or '—'} · {emp.get('grid_zone','—')}</div></div>
                    <div class="spot-score">{score_txt}<small>score</small></div>
                </div>
                """
        st.markdown(f"""
            <div class="yc-panel">
                <div class="yc-panel-title"><h3>Top Talent Spotlight</h3><span>2025</span></div>
                {spot_html}
            </div>
        """, unsafe_allow_html=True)
    ...
    with col_heat:
        ...
        for code in CAT_ORDER:
            ...
            for val in vals:
                ...
                cells += f'<td style="background:rgba(102,110,255,{alpha:.2f});color:#101828">{val}</td>'
        ...
        st.markdown(f"""
            <div class="yc-panel">
                <div class="yc-panel-title"><h3>Talent Heatmap</h3><span>9-Grid density</span></div>
                {heat_html}
            </div>
        """, unsafe_allow_html=True)
    ...
    with col_ins:
        ...
        if not y2025.empty and "dept" in y2025.columns:
            ...
            if not star_depts.empty:
                top_dept = star_depts.value_counts().idxmax()
        st.markdown(f"""
        <div class="insight-box">
            <h3>AI-style Executive Insights</h3>
            <ul>
                <li><b>{stars_pct:.1f}%</b> of the 2025 evaluated population is classified as Stars.</li>
                <li>Risk exposure is currently <b>{risk_pct:.1f}%</b> of the evaluated population.</li>
                <li>Average performance changed by <b>{avg_change:+.2f}</b> points vs 2023.</li>
                <li><b>{top_dept}</b> produced the highest number of Stars in 2025.</li>
                <li><b>{improved}</b> employees improved their grid position from 2024 to 2025.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
